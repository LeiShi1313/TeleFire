...
## Market challenge update, 2026-07-17

The updated search found several credible newer systems, but **no unqualified better self-hosted replacement for Telefire's current Hindsight core**. "Better" changes by workload:

- **Zep is the strongest managed replacement.** It has temporal context graphs, explicit group-chat ingestion through standalone graphs, facts/entities/episodes/observations, hard graph isolation, custom entity and edge types, enterprise authorization, and a documented sub-200 ms retrieval target. It is not an ordinary local deployment: Zep Community Edition is deprecated, leaving Zep Cloud, enterprise BYOC, or the lower-level Graphiti library. Its documentation also says multilingual graph creation is not yet explicit product support. See [Zep concepts](https://help.getzep.com/concepts) and [Zep FAQ](https://help.getzep.com/faq).
- **Honcho v3 is the strongest multi-person challenger.** Peers can be people, agents, groups, projects, or ideas; sessions can contain many peers; and representations can model both a peer and one peer's observations of another. Its asynchronous Deriver and Dreamer produce explicit, deductive, and inductive conclusions, reconcile stale conclusions, and can trace conclusions to supporting messages. This maps naturally to group conversations, but it is peer-centric rather than bank/world-centric, its normal context result exposes less direct source evidence than Hindsight recall, and self-hosting requires an API service, worker, PostgreSQL/pgvector, and Redis. The core is AGPL-3.0. See [architecture](https://honcho.dev/docs/v3/documentation/core-concepts/architecture), [representations](https://honcho.dev/docs/v3/documentation/core-concepts/representation), and [self-hosting](https://honcho.dev/docs/v3/contributing/self-hosting).
- **MemMachine is the strongest ground-truth-preserving recall challenger.** It keeps raw single-message episodes, expands relevant hits with surrounding episodes, and separately extracts profile features whose citation table points back to source episode IDs. Its April 2026 paper reports 91.69 on LoCoMo, 93.0 on LongMemEval-S, and roughly 80% fewer input tokens than Mem0 under matched conditions. That is credible evidence for contextual episode retrieval, but not yet for Telefire's relationship and isolation requirements: the open-source graph documentation is incomplete, organization/project/group/user/session IDs are logical query context rather than a documented authorization boundary, and local operation requires both Neo4j and SQL storage. See the [paper](https://arxiv.org/abs/2604.04853), [memory types and citations](https://github.com/MemMachine/MemMachine/blob/main/docs/open_source/memory_types.mdx), and [repository](https://github.com/MemMachine/MemMachine).
- **MatrixOrigin Memoria is the strongest isolation and versioning watch candidate.** Its current multi-database design physically separates each user database, its group proposal uses one physical database per group with API-key-bound membership, and it exposes correction, snapshots, branches, merge, rollback, entity extraction, graph search, REST, and MCP. This is unusually close to the security and repair semantics Telefire eventually wants. It is not ready to displace Hindsight: v0.4.0 is young, depends on MatrixOne, publishes hand-authored graph scenarios rather than comparable LoCoMo/BEAM or latency results, and intentionally grants anonymous callers master privileges when deployed without a master key. See the [per-user database architecture](https://github.com/matrixorigin/Memoria/blob/main/docs/per-user-database-architecture.md), [group collaboration design](https://github.com/matrixorigin/Memoria/blob/main/docs/specs/2026-04-12-group-collaboration-api.md), [graph benchmark rationale](https://github.com/matrixorigin/Memoria/blob/main/docs/graph-benchmark-rationale.md), and [authentication notes](https://github.com/matrixorigin/Memoria/blob/main/memoria/crates/memoria-api/AUTH_CHANGES.md).
- **Supermemory local is the strongest compact operational challenger.** Its new single binary includes an embedded graph engine, local or OpenAI-compatible embeddings, OpenAI-compatible LLM extraction, raw document ingestion, versioned memories, `UPDATES`/`EXTENDS`/`DERIVES` links, and strict container-tag vector namespaces. The local server is nevertheless at `server-v0.0.5`, has one API key, logs instead of the hosted control dashboard, and uses the caller's model rather than the proprietary extraction models behind the hosted product's quality claims. More importantly, the public repository is MIT-licensed but does not contain the server implementation visible in the release binaries, so the local engine is not currently inspectable like Hindsight. Search results also do not directly expose Hindsight-style fact-to-chunk evidence. See [self-hosting](https://supermemory.ai/docs/self-hosting/overview), [local versus enterprise](https://supermemory.ai/docs/self-hosting/local-vs-enterprise), [graph memory](https://supermemory.ai/docs/concepts/graph-memory), and [container tags](https://supermemory.ai/docs/concepts/container-tags).
- **OpenViking is the strongest broader context-database challenger.** It unifies memory, resources, and skills under inspectable `viking://` paths, progressively loads L0/L1/L2 context, archives raw session messages, records add/update/delete memory diffs, supports images and tools, and has account/user/peer isolation. This is attractive if Telefire wants to replace more than memory. It is not currently a better relationship-memory core: its general memory-link layer is still documented as a draft, its model is user-plus-peers rather than a shared world bank, and its current LoCoMo results are below Hindsight's. The main project is AGPL-3.0 even though some documentation footers say Apache-2.0. See [architecture](https://docs.openviking.ai/en/concepts/01-architecture), [sessions and memory diffs](https://docs.openviking.ai/en/concepts/08-session), [multi-tenancy](https://docs.openviking.ai/en/concepts/11-multi-tenant), the [draft relation design](https://docs.openviking.ai/design/memory-link-design), and the repository's [license declaration](https://github.com/volcengine/OpenViking#license).

### Updated decision table

| Choice | Where it is better than Hindsight | Decisive gap for Telefire | Action |
|---|---|---|---|
| Hindsight | Best combined evidence, time, relationship retrieval, observations, bank isolation, and current public large-scale quality | Retain and consolidation are LLM-bound; exact external entity identity still needs adapter discipline | **Keep as core** |
| Zep | Managed operations, governance, group-chat graph API, enterprise scale | No supported ordinary self-host; BYOC is enterprise; current multilingual caveat | Best managed alternative only |
| Honcho | Multi-peer sessions and perspective-aware representations | Heavier AGPL stack; peer/workspace model does not directly implement controlled cross-bank world memory; weaker evidence surface | Run a targeted group-memory pilot, not a migration |
| MemMachine | Strong published conversational recall, raw episode preservation, source-linked profile facts | No demonstrated hard service isolation or shared-world relationship graph; Neo4j plus SQL stack | Run the primary recall/evidence bake-off |
| MatrixOrigin Memoria | Physical user/group database isolation, correction, snapshots, branches, and rollback | Young MatrixOne-dependent service; synthetic graph suite; dangerous open mode without a master key | Watch; pilot only for isolation/versioning |
| Supermemory local | Simplest deployment, multimodal document ingest, explicit memory update graph | Very new binary, server source absent from public tree, local quality unproven, single-tenant controls | Operations spike only after API/source review |
| OpenViking | Unified memory/resource/skill context, progressive loading, auditable file projection | Relationship layer is draft; awkward shared-bank mapping; AGPL; fast-moving release line | Revisit only if replacing the broader context stack |
| Graphiti | Most controllable bitemporal graph substrate | Telefire would have to build the service, consolidation, auth, UI, and policy layer | Keep as lower-level reference |
| Cognee | Broad document and knowledge platform with relational/vector/graph stores | More operational weight and a document-platform center of gravity | Conditional platform pilot |
| Mem0 OSS v3 | Mature integrations, strong latency, high simple-memory benchmark scores | No traversable relationship graph in v3, ADD-only extraction, weaker evidence lifecycle | Do not use as graph-memory core |

### Benchmark evidence and limits

Public benchmark numbers are useful for rejecting weak candidates, but they do not select the production engine. Vendors use different answer models, judges, retrieval cutoffs, token budgets, and sometimes different versions of a benchmark. None of the common leaderboards tests Telefire's hardest requirements: multi-speaker attribution in Chinese, canonical identity, explicit bank grants, cross-bank directory discovery, evidence drill-down, write throughput, or isolation under adversarial queries.

| System and published run | LoCoMo | LongMemEval S | BEAM 1M | BEAM 10M | Important qualification |
|---|---:|---:|---:|---:|---|
| [Hindsight](https://benchmarks.hindsight.vectorize.io/) | 92.0 | 94.6 | 73.9 | 64.1 | Vendor page links to Agent Memory Benchmark results; no latency-by-bank-size curve is published |
| [Mem0 April 2026 algorithm](https://mem0.ai/research) | 92.5 | 94.4 | 64.1 | 48.6 | Vendor run; about 6.7K-7.0K retrieved tokens and roughly 0.9-1.1 s p50; current OSS extraction is ADD-only |
| [MemMachine paper](https://arxiv.org/abs/2604.04853) | 91.69 | 93.0 | - | - | Preserves raw episodes and reports about 80% fewer input tokens than Mem0; no comparable million-scale or hard-isolation evaluation |
| [Honcho reproducible December 2025 run](https://github.com/plastic-labs/honcho-benchmarks) | 89.9 | 90.4 | 63.1 | 40.6 | Uses multiple configured models and an agentic Dialectic; repository includes result artifacts and configs |
| [OpenViking 0.3.22](https://github.com/volcengine/OpenViking#evaluation-highlights) | 80.3-82.9 | - | - | - | End-to-end agent integrations, not an apples-to-apples memory-only run; query time was 20.4-38.8 s |

The local evidence matters more for the current bottleneck. Telefire's measured 2,000-message Hindsight backfill took about 81 minutes: 621 extraction calls consumed 4,313 seconds, local embeddings 183 seconds, and database writes 4.4 seconds. The problem is overwhelmingly extraction-model throughput, not Hindsight storage. Changing the backend will help only if it materially reduces extraction calls or uses a faster extraction strategy; see [the measured performance specification](../specs/hindsight-memory-model-performance.md).

### Managed cloud options

The large cloud vendors now have credible managed memory primitives, but neither replaces Telefire's graph semantics:

- [Amazon Bedrock AgentCore Memory](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/memory.html) provides short-term events plus semantic, summarization, preference, episodic, and custom long-term strategies. Actor/session namespaces and IAM can enforce strong isolation. It is a good managed personalization store, not a first-class temporal relationship graph.
- [Vertex AI Agent Engine Memory Bank](https://cloud.google.com/vertex-ai/generative-ai/docs/agent-engine/memory-bank/fetch-memories) supports immutable exact-match scopes, configurable extraction topics and examples, consolidation, TTL, and similarity retrieval. It remains a preview managed service and returns fact memories rather than an inspectable relationship/evidence graph.

### Research watch list

[SelfMem](https://arxiv.org/abs/2607.03726) is the most important new research direction, not a deployable market replacement. It keeps the raw transcript immutable and lets an agent decide what to store, revise, compress, or leave for source lookup. Under one shared BEAM harness it improved the strongest baseline score by 40.8-48.7% from 100K to 1M tokens. The result argues for making Telefire's **agent retrieval policy** more adaptive over time; it does not provide bank authorization, a multi-tenant service, provenance APIs, or an operational database.

Two newer performance papers reinforce that backend replacement is not the only lever. [MemRouter](https://arxiv.org/abs/2605.00356) replaces per-turn LLM admission with a small learned router and reports a drop from 970 ms to 58 ms p50 for the memory-management decision under its matched harness. [DMF](https://arxiv.org/abs/2606.03463) uses deterministic scoring and pruning with zero LLM tokens for context preparation. Neither is a production memory service, but both are relevant to Telefire's measured write-side extraction bottleneck.

Other newly screened systems do not clear the replacement bar:

- [Memori](https://github.com/MemoriLabs/Memori) is a useful BYODB library with entity/process/session attribution, raw conversations, extracted facts, SQL triples, local embeddings, and a low-token LoCoMo run. Its triples are intentionally shared per entity across processes, and it lacks the hard bank, temporal invalidation, and evidence contract required here.
- [ReMe](https://github.com/agentscope-ai/ReMe) is a promising local Markdown memory service with raw sessions, daily cards, dream consolidation, wikilinks, BM25/vector retrieval, HTTP, and MCP. It is better as an inspectable compiled projection or personal-agent memory than as a multi-tenant relationship store.
- Memvid, memU, LycheeMemory, TeleMem, Redis's unsupported OSS research server, MemOS, EverOS, and Neo4j Agent Memory each offer useful components or ideas, but none improves the full Telefire requirement set enough to justify migration.

**Recommendation:** do not migrate now. Keep Hindsight behind Telefire's existing client-neutral memory API. If confidence requires empirical proof, run one primary controlled bake-off on the same Coder Offtopic corpus and Chinese query set: Hindsight versus MemMachine for raw-episode recall and source quality. Add Honcho only if peer-perspective reasoning is the question being tested; add Supermemory only as an operations/throughput spike after its local API and source-distribution model are acceptable. Gate any replacement on attribution, temporal correction, cross-bank isolation, evidence quality, abstention, ingest cost, and p50/p95 recall latency rather than a vendor headline score.

**Confidence:** high that Hindsight remains the best overall fit from documented products; medium on relative retrieval quality until the local corpus bake-off is run; low on vendor latency comparisons because no shared scale-and-latency harness covers these systems.
...
| [Honcho](https://github.com/plastic-labs/honcho) v3 | Strong, native multi-peer sessions | Partial to strong internally through messages, premises, and reasoning chains | Partial: peer representations and hybrid conclusion search, not a general entity graph | Strong with workspace-per-boundary; cross-session peers need policy care | Strong Dialectic agent with configurable limits | Strong but operationally heavy; AGPL-3.0 | Peer-centric reasoning memory | **Targeted group-memory pilot** |
| [MemMachine](https://github.com/MemMachine/MemMachine) | Strong raw episode ingest; profile extraction is user-centric | Strong for raw episodes and source-linked profile facts | Partial: Neo4j episodic context, but general relationship traversal is not documented | Partial: logical org/project/group/user/session context; OSS service auth boundary is not documented | Strong optional retrieval agent; host must cap it | Strong; Apache-2.0, Neo4j plus SQL | Ground-truth-preserving episodic/profile memory | **Primary recall/evidence bake-off** |
| [MatrixOrigin Memoria](https://github.com/matrixorigin/Memoria) | Strong multi-type memory and entity extraction | Strong correction and version history; benchmarked provenance quality is unclear | Strong documented entity/edge graph plus hybrid search | Strong physical user/group DB design when auth is configured; fail-open dev mode is unsafe | Partial through reflect/search/MCP tools | Strong; Apache-2.0, MatrixOne-dependent | Versioned graph memory service | Watch; pilot for isolation/versioning after maturity |
| [Supermemory local](https://supermemory.ai/docs/self-hosting/overview) | Strong for conversations and documents | Partial: source documents and version history exist, but normal search lacks direct evidence drill-down | Partial: fact-on-fact update/extend/derive graph plus hybrid search | Strong container namespaces; local auth is one server key | Weak: retrieval API, not a bounded reasoning agent | Strong deployment, but binary implementation is not in the public repository | Compact memory/RAG server | **Targeted local bake-off** |
| [OpenViking](https://github.com/volcengine/OpenViking) | Strong session and multimodal ingest | Partial through raw archives and per-commit memory diffs | Partial: hierarchical retrieval is strong; general memory relations remain draft | Strong account/user/peer isolation | Strong hierarchical search and progressive loading | Strong; AGPL-3.0 | Broad context database | Revisit for a broader context-platform replacement |
...
### MemMachine, Supermemory local, and EverOS

- [MemMachine](https://github.com/MemMachine/MemMachine) is an Apache-2.0 Python service with raw episodic Neo4j memory, SQL profile memory, working memory, REST/SDK/MCP, and organization/project/group/agent/user/session identifiers. Profile features retain citations to source episode IDs, while contextualized retrieval expands a relevant episode with nearby conversation turns. Its [April 2026 paper](https://arxiv.org/abs/2604.04853) reports 91.69 LoCoMo, 93.0 LongMemEval-S, and roughly 80% fewer input tokens than Mem0 under matched conditions. This makes it a credible recall/evidence challenger. It remains weaker for Telefire's complete target because its open-source relationship-graph documentation is unfinished, its identifiers are not a documented hard authorization boundary, and its architecture is centered on personalized user/session memory rather than independently granted shared banks.
- [Supermemory local](https://supermemory.ai/docs/self-hosting/overview) became materially more relevant after the first survey. The July 2026 server is a single self-contained binary with an embedded fact graph, hybrid memory/document retrieval, strict container namespaces, local embeddings, and OpenAI-compatible extraction. It is a credible low-operations bake-off candidate. It is not yet the selected core because `server-v0.0.5` is very new, the server implementation is absent from the public MIT repository even though binaries are released there, local extraction quality has no reproducible result artifact, local operation has one API key and logs rather than a dashboard, and the public search response does not expose Hindsight-style fact-to-source evidence.
...
[MatrixOrigin Memoria](https://github.com/matrixorigin/Memoria) is an Apache-2.0 Rust service built on MatrixOne. It combines semantic, profile, procedural, working, and episodic memories with vector/full-text retrieval, extracted entities, typed graph edges, spreading activation, reflection, contradiction handling, correction, feedback, REST, and MCP. Its unusual strength is versioned isolation: the implemented [per-user database architecture](https://github.com/matrixorigin/Memoria/blob/main/docs/per-user-database-architecture.md) gives every user a physical database so snapshots, branches, restore, and rollback operate at the right boundary; its [group collaboration design](https://github.com/matrixorigin/Memoria/blob/main/docs/specs/2026-04-12-group-collaboration-api.md) extends that model to a physical database per group and API-key-bound group membership.

That architecture is promising, but its evidence is not yet comparable to Hindsight. The published graph suite is a set of ten hand-authored expected scenarios for disambiguation, multi-hop retrieval, temporal chains, and opinion evolution, not a measured LoCoMo/LongMemEval/BEAM run. Version v0.4.0 is young, MatrixOne is a substantial storage dependency, and the API's documented open development mode deliberately treats anonymous callers as master when no master key is configured. It is a serious watch candidate for isolation and repair semantics, not the current production selection.
...
| **Targeted bake-off only** | [MemMachine](https://github.com/MemMachine/MemMachine), [Honcho](https://github.com/plastic-labs/honcho), [Supermemory local](https://supermemory.ai/docs/self-hosting/overview) | MemMachine is the primary ground-truth recall/evidence challenger; Honcho tests peer-perspective reasoning; Supermemory is only an operations/throughput spike |
...
| **Watch / borrow ideas** | [MatrixOrigin Memoria](https://github.com/matrixorigin/Memoria), [OpenViking](https://github.com/volcengine/OpenViking), [ReMe](https://github.com/agentscope-ai/ReMe), [SelfMem](https://arxiv.org/abs/2607.03726), [MemRouter](https://arxiv.org/abs/2605.00356), [DMF](https://arxiv.org/abs/2606.03463), [TencentDB Agent Memory](https://github.com/TencentCloud/TencentDB-Agent-Memory), [LLM Wiki](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f), [Nowledge Mem](https://mem.nowledge.co/docs), [LangMem](https://github.com/langchain-ai/langmem), [Letta](https://github.com/letta-ai/letta), [MemOS](https://github.com/MemTensor/MemOS), [GraphRAG](https://github.com/microsoft/graphrag) | Physical isolation/versioning, progressive loading, inspectable synthesis, adaptive write/retrieval policy, score/source UX, fixed namespaces, memory hierarchy, capability sets, and community retrieval are useful without adopting each runtime |
