...
- Focused Dream tests use a fake Telegram history source and a controlled memory endpoint. They prove deterministic segment packing, actor and reply preservation, legacy document stability, ancestors outside the window, bounded overlap, newest-message truncation, settlement delay, lossy watermark advancement, partial failure retry, per-scope leases, four-way retention, restart recovery, and bounded rate limiting.
