...
- Dream fetches the oldest unprocessed messages after its durable cursor and refreshes a separately bounded overlap for edits. It checkpoints only the completed chronological prefix after each retain wave, stops launching new waves after its soft cycle budget, and resumes the remainder later. Failed or incomplete work therefore remains retryable without discarding already accepted progress. A renewable per-scope lease prevents overlapping cycles.
...
- Focused Dream tests use a fake Telegram history source and a controlled memory endpoint. They prove deterministic segment packing, actor and reply preservation, legacy document stability, ancestors outside the window, bounded overlap and oldest-first catch-up, settlement delay, incremental cursor checkpoints, partial failure retry, per-scope leases, four-way retention, restart recovery, and bounded rate limiting.
